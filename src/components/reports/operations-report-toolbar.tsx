...
import type { OperationsReportRange } from "@/modules/reports/operations-schemas";

const ranges: Array<{ value: OperationsReportRange; label: string }> = [
  { value: "30d", label: "30 days" },
  { value: "90d", label: "90 days" },
  { value: "365d", label: "12 months" },
];

export function OperationsReportToolbar({
  range,
  selectedStoreId,
  stores,
}: {
  ...
  function update(values: { range?: OperationsReportRange; store?: string }) {
    const next = new URLSearchParams(searchParams.toString());
    if (values.range) {
      if (values.range === "90d") next.delete("range");
  ...
  return (
    <div className="flex flex-col gap-4 p-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
      ...
      <div className="relative min-w-0 sm:w-64">
        ...
        <SelectField
          ariaLabel="Operations report store"
