...
The UI can be previewed without MongoDB using its deterministic development demo, including read-only product details and category taxonomy. Live tenants can create, inspect, edit, and archive products at `/app/{tenantSlug}/products`, and manage categories at `/app/{tenantSlug}/products/categories`; mutations, authentication, billing projections, platform security, and tenant isolation tests require the replica set. Catalog edits use optimistic concurrency. Product archive preserves inventory, while category archive is blocked until active/draft assignments are retired. The tenant billing console is available at `/app/{tenantSlug}/settings/billing`; without Stripe test credentials it remains safely read-only.
