...
The UI can be previewed without MongoDB using its deterministic development demo, including read-only product details, images, categories, tags, option groups, variants, and inventory history. Live tenants can create, inspect, edit, and archive products at `/app/{tenantSlug}/products`, import and export filtered catalog data on Growth or higher plans, manage Cloudinary-hosted product galleries from each product detail screen, manage categories at `/app/{tenantSlug}/products/categories`, manage reusable product tags at `/app/{tenantSlug}/products/tags`, configure product-specific options and sellable variants, and run store-scoped inventory at `/app/{tenantSlug}/inventory`. Inventory includes low-stock visibility, immutable adjustments, multi-line inter-store transfers, optimistic stock checks, atomic projections and ledger events, idempotent retry, and audit history. The CSV workflow includes a downloadable template, explicit column mapping and existing-SKU policy, row-level preview/validation, a downloadable error report, and an atomic audited commit. Mutations, authentication, billing projections, platform security, and tenant isolation tests require the replica set. Catalog edits use optimistic concurrency. Product and zero-stock variant archive preserve inventory records, while category, tag, option, image, import, and stock safeguards protect lifecycle invariants. Product image uploads remain safely disabled until all three server-only `CLOUDINARY_*` credentials are configured. The tenant billing console is available at `/app/{tenantSlug}/settings/billing`; without Stripe test credentials it remains safely read-only.
