...
The UI can be previewed without MongoDB using its deterministic development demo, including read-only product details, images, categories, tags, option groups, and variants. Live tenants can create, inspect, edit, and archive products at `/app/{tenantSlug}/products`, manage Cloudinary-hosted product galleries from each product detail screen, manage categories at `/app/{tenantSlug}/products/categories`, manage reusable product tags at `/app/{tenantSlug}/products/tags`, and configure product-specific options and sellable variants. Mutations, authentication, billing projections, platform security, and tenant isolation tests require the replica set. Catalog edits use optimistic concurrency. Product and zero-stock variant archive preserve inventory records, while category, tag, option, and image safeguards protect lifecycle invariants. Product image uploads remain safely disabled until all three server-only `CLOUDINARY_*` credentials are configured. The tenant billing console is available at `/app/{tenantSlug}/settings/billing`; without Stripe test credentials it remains safely read-only.
