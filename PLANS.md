...
- Ran all eleven database migrations and the deterministic seed against MongoDB 8, including integration coverage for overlapping-tenant repository isolation, atomic onboarding, authorized workspace projections, cross-tenant store rejection, invitation grant activation, verified billing lifecycle projection, category/tag/variant/image lifecycles, and platform security boundaries.
...
- Ran all eleven database migrations and validated platform bootstrap/access/aggregate boundaries in integration tests plus TOTP enrollment, stale-session rejection, re-verification, fresh login, and accessibility on desktop and mobile.
...
- Complete CSV preview/import/export, inventory adjustment, and stock transfer workflows.
...
Implement tenant-scoped CSV preview/import/export with explicit column mapping, row-level validation, bounded job contracts, safe formula handling, and audited product upserts.
