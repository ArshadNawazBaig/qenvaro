...
- Ran all fourteen database migrations and the deterministic seed against MongoDB 8, including integration coverage for overlapping-tenant repository isolation, atomic onboarding, authorized workspace projections, cross-tenant store rejection, invitation grant activation, verified billing lifecycle projection, category/tag/variant/image/CSV/inventory lifecycles, and platform security boundaries.
...
- Ran all fourteen database migrations and validated platform bootstrap/access/aggregate boundaries in integration tests plus TOTP enrollment, stale-session rejection, re-verification, fresh login, and accessibility on desktop and mobile.
...
- Add product/store availability management and actionable low-stock alert preferences.
...
Implement audited product/store availability management and tenant low-stock alert preferences, using the existing reorder levels and authorized-store inventory projections.
