...
flowchart LR
  ...
  Browser <-->|Authenticated Socket.IO| Realtime[Realtime gateway]
  Realtime --> App
...
The dashboard remains a read-heavy Server Component. A dedicated repository receives trusted tenant context and a Zod-bounded 7/30/90/120-day period, aligns calendar buckets to the tenant timezone, and runs indexed sales/audit projections. Active-store trends never broaden beyond the selected authorized store; comparisons are capped to authorized active stores; and activity is both time/row bounded and allowlisted before safe summaries reach the view. Recharts is isolated to a narrow Client Component and receives serializable projection data with an equivalent screen-reader table.

Point of sale uses a Server Component for the tenant/store-scoped catalog and customer projection, a narrow Client Component for the provisional cart interaction, and a Server Action for completion. A dedicated authenticated scan endpoint resolves an exact variant barcode or normalized SKU only inside the active assigned store and returns its live inventory version; USB/Bluetooth keyboard scanners add the result when they send Enter. Browser totals are estimates only. The sale service re-resolves ownership, commercial values, tax, entitlement, availability, and inventory versions; then one MongoDB transaction creates immutable sale/payment/receipt evidence, calls the inventory service, updates revenue, and appends a safe audit. The resulting receipt includes business/store contact details and an 80 mm-class print layout; a same-tab, one-time print request can open it automatically after completion. Recorded payment handling sits behind a provider interface so a future tenant payment integration does not cross-contaminate Stripe subscription billing.

Sales history is a bounded tenant/store-scoped Server Component read. A return starts from the original receipt, where a client workspace selects quantities and shows only an estimate. The Server Action delegates to a return service that re-resolves prior returns and original sale snapshots, derives exact cumulative allocations, records the refund through the payment-provider boundary, restores tracked inventory, and persists return/refund/receipt evidence plus projections and audit in one transaction. This preserves historical SKU returns after catalog archive without allowing current catalog values to rewrite completed commerce. A separately authorized void workflow is limited to completed sales recorded by the manual payment provider. It requires the exact receipt number, is idempotent, rejects sales with completed returns, and atomically restores tracked stock, reverses revenue, voids the sale/payment/receipt evidence, and audits the actor.

Sales reporting is a read-only Server Component over a dedicated repository and shared tenant-calendar period utility. Every aggregation starts with trusted tenant, active assigned-store, status, and bounded date filters. Sales and returns are aggregated independently by their event timestamps and merged into serializable daily, method, store, product, and paginated transaction projections. Recharts remains isolated to a narrow Client Component with a complete screen-reader table; URL period/store controls are the only interactive reporting client boundary. Sales and operations reports expose print views and bounded, formula-safe CSV downloads through authenticated Route Handlers; each export re-runs authorization and stores an `importExportJobs` summary plus an audit event.

Workforce and purchasing use the same service boundary. Employee directories omit compensation, while salary/payroll services require separate permissions and write only safe counts/state to audits. Purchase receiving delegates stock changes to the inventory service inside the same transaction; approved expense reporting reads only immutable approval evidence. Operations-report period and store controls are Zod-bounded and can select only active stores assigned to the member.

Tenant settings keep business identity, regional values, tax/numbering/inventory policy, stores, custom roles, security posture, and controlled data requests in scoped services with optimistic versions. Custom role permissions are code-allowlisted and merged during trusted context resolution only when the current plan includes the feature. Ownership transfer is a dedicated two-factor-protected service operation: it verifies the current owner, target membership, target email, and tenant permission before swapping the fixed roles and emitting audit and notification evidence in one transaction.

The application shell receives only bounded workspace metadata. Its store switcher is populated from every active store allowed by the current membership, its notification popover shows a capped tenant/user-scoped preview, and global search calls a debounced authenticated endpoint. Search results are grouped by products, customers, employees, and settings; each provider is included only when the context has its matching read permission, and every database query begins with tenant and authorized-store scope.

The custom Node HTTP server attaches Socket.IO at `/socket.io`. Every connection presents the same-origin Better Auth cookie plus a tenant slug; a private no-store projection re-runs tenant membership resolution before the socket joins tenant, user, role, and platform-announcement rooms. Notification events are Zod-validated and published only after their durable MongoDB transaction commits. The client validates incoming payloads, displays new-notification toasts, and refreshes the authoritative Server Component projection. Socket delivery is an acceleration layer rather than the source of truth, so reconnects and unavailable transports cannot lose durable notification state.
