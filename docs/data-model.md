...
The sales report is also a read projection. It groups completed sales by `completedAt` and completed returns/refunds by their own processing timestamps across Zod-bounded 7-, 30-, or 90-day tenant-calendar periods. Net sales subtract in-period discounts and returned pre-tax value, while refund totals include reversed tax. Store options begin with the member's active assignments, outputs cap stores at 100, products at eight, transaction pages at 25, and URL pages at 100. Missing sale or return cost evidence suppresses profit and margin. The operations report uses the same assigned-store rule with bounded 30-, 90-, or 365-day periods. Report exports re-run these projections, formula-escape CSV cells, and append an `importExportJobs` summary. Migration 21 adds store/date reporting indexes for returns, recorded sale payments, and refunds; the existing sales store/date index supports the sale side.
...
Product catalog records carry an integer `version`, rich descriptive data, barcode, integer minor-unit price/cost, unit, tax/tracking policy, and service-or-stocked type. Detail and list reads scope products, variants, stores, inventory levels, and revenue to the server-derived tenant context; stock totals and displayed revenue include only stores authorized for the current membership. Catalog edits compare the submitted expected version inside the transaction, update the product and its default variant together, and increment both versions. Service products never create opening inventory. Archive is an idempotent product lifecycle transition: it marks the product and linked variants, appends an audit event, and never changes or removes `inventoryLevels` or `inventoryMovements`.
...
`sales` is a completed commercial record rather than a mutable browser cart. It embeds immutable store, optional customer, and line snapshots plus server-derived subtotal, discount, tax, pre-tax net total, total, tendered amount, change, and nullable gross profit. Separate `salePayments` records retain the recorded method, tendered/applied amounts, provider kind, and status; they never contain credentials. Each issued `receipt` provides a tenant/store-scoped sequential display number and evidence for one sale. The same transaction allocates that number, creates the sale/payment/receipt records, calls the inventory service for each tracked line, updates the product revenue projection, and appends a safe audit. A request fingerprint permits only an exact idempotent replay. A manual-provider sale may later transition from completed to voided only when it has no completed return; the void transaction restores tracked inventory with `sale_void` ledger entries, reverses product revenue, voids its payment and receipt, and retains every commercial record. Migration 18 adds product tax/tracking defaults and sale/payment/receipt indexes; migration 19 adds the bounded variant-first POS catalog index.

`returns` is immutable reversal evidence tied to one completed sale and its original store. Each return embeds original-line identity and product snapshots, returned quantity, and the cumulatively allocated original subtotal, discount, tax, total, and nullable profit reversal. Completed prior returns determine the remaining quantity; cumulative integer allocation ensures repeated partial returns reconcile exactly to the original line without cent drift. A separate `refund` records the selected recorded method and amount, while an entity-typed return `receipt` receives its own store sequence. One transaction writes all three records, appends `sale_return` inventory movements for tracked lines, updates stock and revenue projections, increments explicit returned-value summaries on the sale, and appends a PII-safe audit. Archived historical products and variants can receive stock back, but the original store must remain active and assigned. Migration 20 backfills original receipt types and adds distinct sale/return receipt, idempotency, numbering, refund-evidence, and history indexes.

Products embed up to three option groups with stable option/value identifiers, current display labels, and lifecycle metadata. Sellable `productVariants` reference one value from every active group and store an immutable option signature; label changes resolve at read time without rewriting variant identity. The base/default variant mirrors product SKU, barcode, price, and cost. Additional variants carry independent SKU, optional tenant-unique barcode, price, cost, status, and version fields and start with zero stock. Variant archive is idempotent, retains SKU/combination reservations and history, and is rejected while any tenant inventory projection is non-zero. Option groups cannot be added after active combinations exist, and cannot be archived while active variants reference them. Migration 10 backfills lifecycle fields and creates combination/status indexes; migration 30 adds the partial tenant-unique active variant barcode index.
...
`productImportPreviews` stores only bounded parsed CSV values and validated mutation intent; it does not store a binary file. Every preview belongs to one tenant and user, expires after 30 minutes, carries an optimistic version, and records the chosen mapping and existing-SKU behavior. A successful commit converts the preview into an idempotent completed result and writes a durable `importExportJobs` summary plus audit records in the same transaction as catalog changes. Sales and operations CSV exports also write bounded completed job summaries without storing report rows. New-product opening stock uses the append-only inventory ledger and projection; updates and skips never alter inventory. Migration 12 adds preview TTL and tenant job indexes. `applicationRateLimits` contains short-lived fixed-window counters for application routes such as CSV operations; migration 13 adds TTL cleanup.
...
- unique `tenantId + normalized variant SKU`, partial unique active `tenantId + variant barcode` from migration 30, unique `tenantId + productId + option signature`, and `tenantId + productId + variant status + updatedAt` variant lifecycle queries;
...
- tenant-unique active variant barcodes from migration 30.
